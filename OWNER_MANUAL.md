...
| Neo4j Aura: `Unauthorized` or `Unable to retrieve routing information` despite correct-looking credentials | **Don't assume Aura's username/database are always `neo4j`.** This particular free instance uses the instance ID (e.g. `56d8ad1d`) as *both* `NEO4J_USER` and `NEO4J_DATABASE` — not the standard default. Verify directly with a real connection test (`driver.verify_connectivity()`), not assumption. | Set `NEO4J_USER` and `NEO4J_DATABASE` to the Aura instance ID shown in the console, not the literal string `neo4j` |
| Neo4j warnings like `label does not exist` / `relationship type does not exist` on a fresh Aura database | Harmless — the database is empty and has never had data written to it, so Neo4j is just noting the schema doesn't exist yet | Ignore; it resolves itself once you actually run an extraction that writes data |
